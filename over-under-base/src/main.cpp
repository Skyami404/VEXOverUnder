...
void autonomous(void) {
  ...
  pid_drive(8, 400, 0, 30); // score it //11
  //intake.spin(forward);
  //wait(200, msec);
  intake.spin(forward);
  wait(0.1, sec);
  pid_drive(-1, 800, 0, 30);
  pid_drive(3, 800, 0, 30); //10
  ...
  pid_drive(-1, 300); //goes out to make sure that the robot doesnt touch triball and ->
  pid_drive(1, 300);
  pid_drive(-3);
  //dq scored triballs
